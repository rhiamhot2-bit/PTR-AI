"""Generate a copy-only Rhino 8 Prong Length Correction Boolean Rehearsal."""
...
def classify_boolean_rehearsal(result_count: int, closed: bool, naked_edges: int) -> str:
    ...
    return "PRONG_LENGTH_CORRECTION_BOOLEAN_REHEARSAL_PASSED"


_SCRIPT_TEMPLATE=r'''# -*- coding: utf-8 -*-
# PTR JEW3D Rhino 8 Prong Length Correction Boolean Rehearsal
...
RESULT_LAYER="PTR_PRONG_LENGTH_CORRECTION_BOOLEAN_REHEARSAL"
...
def main():
    ...
    for index,brep in enumerate(results):
        ...
        attributes.Name="PTR_PRONG_LENGTH_CORRECTION_BOOLEAN_REHEARSAL_RESULT_{0}".format(index+1)
    ...
    if blockers:
        status="PRONG_LENGTH_CORRECTION_BOOLEAN_REHEARSAL_BLOCKED"

    report={
        "generator":"ptr-prong-length-correction-boolean-rehearsal-v1",
        "status":status,
    ...
    print("PRONG LENGTH CORRECTION BOOLEAN REHEARSAL | status="+status)
    ...
    print("PRONG LENGTH CORRECTION BOOLEAN REPORT | "+REPORT_PATH)
...
def build_prong_length_correction_boolean_rehearsal_script(report_path:Path)->str:
    return _SCRIPT_TEMPLATE.replace(
        "__REPORT_PATH__",json.dumps(str(report_path).replace("\\","/"))
    )


def prepare_prong_length_correction_boolean_rehearsal(memory_root:Path,now:datetime|None=None):
    stamp=(now or datetime.now()).strftime("%Y-%m-%d_%H-%M-%S-%f")
    script_path=memory_root/"Rhino_Scripts"/f"{stamp}_prong_length_correction_boolean_rehearsal.py"
    report_path=memory_root/"Prong_Length_Correction_Boolean_Rehearsals"/f"{stamp}_prong_length_correction_boolean_rehearsal.json"
    return script_path,report_path,build_prong_length_correction_boolean_rehearsal_script(report_path)


def save_prong_length_correction_boolean_rehearsal(script_path:Path,script:str)->None:
