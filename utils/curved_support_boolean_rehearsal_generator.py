"""Generate a duplicate-only Rhino 8 Curved Support Boolean Rehearsal."""
...
def classify_support_boolean(result_count:int,closed:bool,naked_edges:int)->str:
    if result_count==0:
        return "SUPPORT_BOOLEAN_REHEARSAL_FAILED"
    if result_count!=1:
    ...
    return "CURVED_SUPPORT_BOOLEAN_REHEARSAL_PASSED"


_SCRIPT_TEMPLATE=r'''# -*- coding: utf-8 -*-
# PTR JEW3D Rhino 8 Curved Support Boolean Rehearsal
# Boolean uses duplicated Breps only; source document objects remain untouched.
import io
import json
import os
...
RESULT_LAYER="PTR_CURVED_SUPPORT_BOOLEAN_REHEARSAL"
...
def main():
    ...
    source_ids=[seat_id,band_id]+support_ids if not blockers else []
    duplicates=[]
    ...
    results=[]
    if not blockers and len(duplicates)==4:
        try:
    ...
    if not rs.IsLayer(RESULT_LAYER):
        rs.AddLayer(RESULT_LAYER,color=(30,220,170))
    layer_index=sc.doc.Layers.FindByFullPath(RESULT_LAYER,-1)
    result_ids=[]
    checks=[]
    for index,brep in enumerate(results):
        attributes=Rhino.DocObjects.ObjectAttributes()
        attributes.Name="PTR_CURVED_SUPPORT_BOOLEAN_REHEARSAL_RESULT_{0}".format(index+1)
        attributes.LayerIndex=layer_index
        attributes.ObjectColor=System.Drawing.Color.FromArgb(30,220,170)
    ...
    if blockers:
        status="CURVED_SUPPORT_BOOLEAN_REHEARSAL_BLOCKED"

    report={
        "generator":"ptr-curved-support-boolean-rehearsal-v1",
        "status":status,
        "source_seat_id":str(seat_id) if seat_id else None,
        "source_band_id":str(band_id) if band_id else None,
        "source_support_ids":[str(value) for value in support_ids],
    ...
    print("CURVED SUPPORT BOOLEAN REHEARSAL | status="+status)
    print("SOURCE COUNTS | seat={0} | band={1} | curved_supports={2}".format(
        1 if seat_id else 0,1 if band_id else 0,len(support_ids)
    ))
    ...
    print("CURVED SUPPORT BOOLEAN REPORT | "+REPORT_PATH)
...
def build_curved_support_boolean_rehearsal_script(report_path:Path)->str:
    return _SCRIPT_TEMPLATE.replace(
        "__REPORT_PATH__",json.dumps(str(report_path).replace("\\","/"))
    )


def prepare_curved_support_boolean_rehearsal(memory_root:Path,now:datetime|None=None):
    stamp=(now or datetime.now()).strftime("%Y-%m-%d_%H-%M-%S-%f")
    script_path=memory_root/"Rhino_Scripts"/f"{stamp}_curved_support_boolean_rehearsal.py"
    report_path=memory_root/"Curved_Support_Boolean_Rehearsals"/f"{stamp}_curved_support_boolean_rehearsal.json"
    return script_path,report_path,build_curved_support_boolean_rehearsal_script(report_path)


def save_curved_support_boolean_rehearsal(script_path:Path,script:str)->None:
