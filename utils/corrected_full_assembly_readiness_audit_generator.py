"""Generate a report-only Corrected Full Assembly Manufacturing Readiness Audit."""
...
def classify_corrected_readiness(
    result_count:int,
    closed:bool,
    naked_edges:int,
    volume_mm3:float,
    ...
    return "CORRECTED_FULL_ASSEMBLY_READINESS_SCREEN_PASSED"


_SCRIPT_TEMPLATE=r'''# -*- coding: utf-8 -*-
# PTR JEW3D Rhino 8 Corrected Full Assembly Manufacturing Readiness Audit
...
def main():
    ...
    for object_id in rs.AllObjects(select=False,include_lights=False,include_grips=False) or []:
        ...
        if upper.startswith("PTR_CORRECTED_FULL_METAL_ASSEMBLY_BOOLEAN_RESULT_"):
            ...
            band_id=object_id
        elif upper.endswith("_LENGTH_CORRECTION_TRIAL"):
    ...
    blockers=[]
    if len(result_ids)!=1:
        blockers.append("Exactly one Corrected Full Metal Assembly Boolean result is required.")
    if band_id is None:
        blockers.append("One original Ring Band is required as center reference.")
    if len(trial_prong_ids)!=4:
        blockers.append("Four validated length-corrected trial Prongs are required.")
    ...
    if blockers:
        status="CORRECTED_FULL_ASSEMBLY_READINESS_AUDIT_BLOCKED"

    report={
        "generator":"ptr-corrected-full-assembly-readiness-audit-v1",
        "status":status,
    ...
    print("CORRECTED FULL ASSEMBLY READINESS AUDIT | status="+status)
...
def build_corrected_full_assembly_readiness_audit_script(report_path:Path)->str:
    return _SCRIPT_TEMPLATE.replace(
        "__REPORT_PATH__",json.dumps(str(report_path).replace("\\","/"))
    )


def prepare_corrected_full_assembly_readiness_audit(memory_root:Path,now:datetime|None=None):
    stamp=(now or datetime.now()).strftime("%Y-%m-%d_%H-%M-%S-%f")
    script_path=memory_root/"Rhino_Scripts"/f"{stamp}_corrected_full_assembly_readiness_audit.py"
    report_path=memory_root/"Corrected_Full_Assembly_Readiness_Audits"/f"{stamp}_corrected_full_assembly_readiness_audit.json"
    return script_path,report_path,build_corrected_full_assembly_readiness_audit_script(report_path)


def save_corrected_full_assembly_readiness_audit(script_path:Path,script:str)->None:
