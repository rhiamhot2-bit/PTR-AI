"""Generate a clean review-only Production Candidate from the full assembly result."""
...
def candidate_status(
    ...
    if source_count!=1:
        return "PRODUCTION_CANDIDATE_BLOCKED_SOURCE_COUNT"
    if not closed or naked_edges:
        return "PRODUCTION_CANDIDATE_BLOCKED_TOPOLOGY"
    if volume_mm3<=0:
        return "PRODUCTION_CANDIDATE_BLOCKED_VOLUME"
    if stone_collision:
        return "PRODUCTION_CANDIDATE_BLOCKED_STONE_COLLISION"
    return "PRODUCTION_CANDIDATE_REVIEW_CREATED"


_SCRIPT_TEMPLATE=r'''# -*- coding: utf-8 -*-
# PTR JEW3D Rhino 8 Clean Production Candidate
# REVIEW ONLY: copies one passed Full Assembly result; never exports or deletes.
import io
import json
import os
...
CANDIDATE_LAYER="PTR_PRODUCTION_CANDIDATE_REVIEW"
...
def main():
    ...
    if blockers:
        status="PRODUCTION_CANDIDATE_BLOCKED_PREREQUISITES"
    ...
    candidate_id=None
    if status=="PRODUCTION_CANDIDATE_REVIEW_CREATED":
        duplicate=source_brep.DuplicateBrep()
        attributes=Rhino.DocObjects.ObjectAttributes()
        attributes.Name="PTR_PRODUCTION_CANDIDATE_REVIEW_ONLY"
        attributes.LayerIndex=layer_index
        attributes.ObjectColor=System.Drawing.Color.FromArgb(245,190,40)
        attributes.ColorSource=Rhino.DocObjects.ObjectColorSource.ColorFromObject
        attributes.SetUserString("PTR_STATUS","REVIEW_ONLY")
    ...
    report={
        "generator":"ptr-clean-production-candidate-v1",
        "status":status,
    ...
    print("CLEAN PRODUCTION CANDIDATE | status="+status)
    print("SOURCE ASSEMBLY | count={0} | closed={1} | naked_edges={2} | volume_mm3={3:.3f} | stone_collision={4}".format(
        len(assembly_ids),closed,naked,volume,stone_hit
    ))
    print("CANDIDATE | id={0} | layer={1} | stone_included=NO".format(
        str(candidate_id) if candidate_id else "NONE",CANDIDATE_LAYER
    ))
    ...
    print("PRODUCTION CANDIDATE REPORT | "+REPORT_PATH)
...
def build_clean_production_candidate_script(report_path:Path)->str:
    return _SCRIPT_TEMPLATE.replace(
        "__REPORT_PATH__",json.dumps(str(report_path).replace("\\","/"))
    )


def prepare_clean_production_candidate(memory_root:Path,now:datetime|None=None):
    stamp=(now or datetime.now()).strftime("%Y-%m-%d_%H-%M-%S-%f")
    script_path=memory_root/"Rhino_Scripts"/f"{stamp}_clean_production_candidate.py"
    report_path=memory_root/"Production_Candidate_Reviews"/f"{stamp}_clean_production_candidate.json"
    return script_path,report_path,build_clean_production_candidate_script(report_path)


def save_clean_production_candidate(script_path:Path,script:str)->None:
